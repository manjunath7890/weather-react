...
function WeatherForecast(props) {
  ...
  const sendEmail = (data) => {
    const emailData = {
      to: props?.user, 
      subject: 'Rain fall in : ' + data.resolvedAddress,
      text: 'Alerts: ' + data.description 
    };
  
    fetch("https://weather-server-pi.vercel.app/send-email", {
      method: "POST",
      headers: {
          ...
          <Box width={"40rem"} paddingX={"0rem"}>
            <form onSubmit={handleIdSubmit}>
              <Box display={"flex"} alignContent={"center"} mt={"1rem"}>
                ...
                >
                  <AccountCircleIcon style={{ fontSize: "1.7rem", marginRight:'0.3rem' }}/>  Signup
                </Button>
                <Button
                  component={Link}
                  to="/predict"
                ...
                >
                  <OnlinePredictionIcon style={{ fontSize: "1.7rem"}}/>
                </Button>
              </Box>
            </form>
            <LocationComponent />
